...
- PR-safe one-click demo scenarios are available under `POST /api/demo/pr-scenarios/start`, `POST /api/demo/pr-scenarios/{sessionId}/restart-target-service`, and `POST /api/demo/pr-scenarios/{sessionId}/confirm-target-restarted`; they require `REPAIR_GIT_ENABLED=true` and `REPAIR_GITHUB_ENABLED=true`. The selected `faultType` auto-derives `demo/fault/{faultType}` for worktree creation and PR base targeting. They prepare `repair/{sessionId}` in an isolated git worktree under `REPAIR_WORKTREE_ROOT`, so the main checkout stays on `main`. The restart endpoint starts target-service from the worktree for local demos and the frontend falls back to manual restart if it fails.
- PR-safe demo fault branch mapping: `quantity-division-by-zero -> demo/fault/quantity-division-by-zero`, `wrong-quote-route -> demo/fault/wrong-quote-route`, `wrong-error-status -> demo/fault/wrong-error-status`, `precision-loss -> demo/fault/precision-loss`, `race-condition -> demo/fault/race-condition`, `path-traversal -> demo/fault/path-traversal`.
...
- PR-safe demo readiness is exposed through `GET /api/demo/pr-scenarios/readiness?faultType=...`; it returns LLM/Git/GitHub/Feishu/base-branch/worktree booleans and warnings without exposing secrets, and now checks that the auto-derived base branch is visible locally or as a fetched remote ref.
...
- Current Agent maturity: the workflow is a deterministic Java DAG (`AgenticRepairRunner`) that calls four LangChain4j AI sub-agents (`AgenticDiagnosisAgent`, `AgenticPlanAgent`, `AgenticPatchAgent`, `AgenticReflectionAgent`) for language tasks plus non-AI components for evidence, patch apply, tests, review, commit, PR, Feishu, and records. There is no `SupervisorAgent` or orchestration LLM call. Missing LLM configuration or unrecoverable invalid typed model output after one retry publishes a repair `ERROR` and writes a minimal error record. Current mainline `target-service` is in the repaired state and includes fixed regression surfaces for precision, inventory concurrency, and file path validation.
...
- All AI sub-agents return strongly typed records (`DiagnosisResult`, `RepairPlan`, `PatchProposal`, `RepairReflection`) with LangChain4j `@Description` field annotations; Java parser/operators validate typed objects and retry once before surfacing `ERROR`. `PatchProposalOutputRecoverer` can recover provider outputs that contain base64 raw JSON with unescaped multiline `oldText` / `newText` strings.
...
- For real PR demos, do not inject a temporary fault on `main` and expect a useful PR diff. `main` stays repaired. Use committed `demo/fault/{faultType}` branches that are current `main` plus only that one intentional fault; the PR-safe API derives the selected base branch from `faultType`.
...
Repair is implemented as a deterministic Java DAG with four LangChain4j AI sub-agents, dynamic few-shot injection, and an optional scheduled monitor:
...
- Dynamic few-shot: `FewShotService` ranks past successful repair records by similarity and injects the top-3 into the Patch Agent prompt.
...
- Persist repair records and reflection for future retrieval.
- `ScheduledRepairTrigger` optionally polls target-service health and auto-triggers repair on consecutive failures.
...
10. Dynamic few-shot injection (`FewShotService`) ranks past FIXED repair records by exception-name / file-name similarity and injects top-3 examples into the Patch Agent prompt. The hardcoded few-shot examples in `AgenticPatchAgent` SystemMessage are retained as fallback when no historical records exist.
11. Scheduled auto-monitoring (`ScheduledRepairTrigger`) polls target-service via `/api/orders/quote` and checks for new traceback files every N seconds; triggers repair after 2 consecutive failure detections. Enabled via `REPAIR_MONITOR_ENABLED=true`.
12. Test command is no longer hardcoded: `PlanParserOperator` and `PatchParserOperator` accept any non-blank `testCommand` / `testsToRun` from the AI agents.
13. Three additional real-world e-commerce fault scenarios: `precision-loss` (double-based discount calculation), `race-condition` (unsynchronized inventory deduction), `path-traversal` (unvalidated file download path). Mainline includes the fixed service/controller/repository/test surfaces; `mvn -pl target-service test` currently runs 14 tests.
14. Frontend upgraded: Inter font (UI), JetBrains Mono (code), indigo-violet brand gradient sidebar.
15. Next work: validate each supported fault type once through the PR-safe scenario API, keep one real PR + Feishu golden path, and record the official demo from the Vue console.
